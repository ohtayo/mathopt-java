...
import jp.ohtayo.mathopt.core.EvaluatedValues;
...
public class ZDT3_Constraint {
	 ...
	 * @return 目的関数の値
	 */
	public static EvaluatedValues execute(double[] variable, int threadNumber)
	{
		...
		EvaluatedValues ev = new EvaluatedValues(2, 1);
		ev.fitness = fitness;
		if(fitness[1]>0){
			ev.constraintViolation[0] = 0;
		}else{
			ev.constraintViolation[0] = Math.abs(fitness[1]);
		}
		return ev;
	}
